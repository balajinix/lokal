...
upload_link_caption = "Participate in planning for Bengaluru's future. Demand that Revised Master Plan (RMP) 2031, be prepared by BMPC (Bengaluru Metropolitan Planning Committee) and not the BDA (Bengaluru Devolopment Authority). This post on Bengaluru LOKal page has all the information that you need."
...
for page in pages:
  ...
  try:
    ...
    r = page_graph.post(path=path_string,message=upload_link_caption,link=image_url)
    print r
    link_id = r['id'] 
    print 'Success: posted link with id: ' + link_id
    upload_hash[page_id] = link_id
  except:
    print "Could not upload link to page %s with id: %s and access_token: %s" % (page_name, page_id, access_token)
    dump_file()
  count += 1
